...
export function deriveEpgUrls(playlistUrl: string): string[] {
  // Derived guides come only from Xtream get.php/player_api http(s) URLs.
  // Local file:// playlists derive nothing: relative `url-tvg` references
  // inside a local file are never resolved into file or network reads.
  if (!/^https?:\/\//i.test(playlistUrl.trim())) return [];
  try {
