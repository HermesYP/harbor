...
export function LiveView({ active }: { active: boolean }) {
  ...
  const { state, refresh: refreshPlaylist } = useIptvPlaylist(active ? activeSource : null);
  // Bumping this key forces the EPG reload alongside the playlist refresh, so
  // edits to a local .m3u/.xml source show up from the Refresh button too.
  const [epgRefreshKey, setEpgRefreshKey] = useState(0);
  const refresh = useCallback(() => {
    refreshPlaylist();
    setEpgRefreshKey((n) => n + 1);
  }, [refreshPlaylist]);
  ...
  const { index: baseEpg, error: epgError } = useEpg(
